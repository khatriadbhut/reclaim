...
document.getElementById("googleSignInBtn").addEventListener("click", async () => {
  ...
  btn.disabled = true;
  btn.replaceChildren();
  const signing = document.createElement("span");
  signing.style.fontFamily = "'DM Mono',monospace";
  signing.style.fontSize = "13px";
  signing.style.color = "var(--muted)";
  signing.textContent = "signing in...";
  btn.appendChild(signing);
  ...
  try {
    ...
    if (user.picture) {
      ...
      img.src = String(user.picture);
      img.alt = user.name ? String(user.name) : "";
      avatarWrap.appendChild(img);
    } else if (user.name) {
      const div = document.createElement("div");
      div.className = "user-avatar-placeholder";
      div.textContent = String(user.name).slice(0, 1).toUpperCase();
...
document.getElementById("locationModalIp").addEventListener("click", async () => {
  ...
  btn.appendChild(document.createTextNode(" Location detected"));
});
...
document.getElementById("locationBtn").addEventListener("click", async () => {
  ...
  btn.replaceChildren();
  const detecting = document.createElement("span");
  detecting.style.fontFamily = "'DM Mono',monospace";
  detecting.style.fontSize = "13px";
  detecting.style.color = "var(--muted)";
  detecting.textContent = "detecting location...";
  btn.appendChild(detecting);
  btn.disabled = true;
  ...
  navigator.geolocation.getCurrentPosition(
    ...
    async (err) => {
      if (err.code === err.PERMISSION_DENIED) {
        ...
        pin.textContent = "📍";
        btn.appendChild(pin);
        btn.appendChild(document.createTextNode(" Allow Location Access"));
        showLocationDeniedModal();
      } else {
